...
def generate_reference():
    # Load index
    reference = {"version": "latest", "generated": datetime.now().isoformat(), "standards": {}, "quick_answers": {}}

    # Parse STANDARDS_INDEX.md for quick lookups
    with open("docs/guides/STANDARDS_INDEX.md") as f:
        content = f.read()
    ...
    for line in content.split("\n"):
        if "|" in line and "CS:" in line or "SEC:" in line or "TS:" in line:
            ...
            if len(parts) >= 3:
                code = parts[0].strip("`")
                if code and ":" in code:
                    reference["standards"][code] = {"section": parts[1], "summary": parts[2]}
