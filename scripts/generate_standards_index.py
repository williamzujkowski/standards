...
def extract_sections(content):
    ...
    for line in content.split("\n"):
        ...
        if (
            ...
            and not line.startswith("## Overview")
        ):
    ...
    for filename, code in STANDARD_CODES.items():
        ...
        try:
            ...
            doc_title = (
                title_match.group(1)
                if title_match
                else filename.replace("_", " ").replace(".md", "")
            )
...
""".format(
    ...
    print(
        f"Generated STANDARDS_INDEX.md with {sum(len(sections) for sections in category_data.values())} sections"
    )
