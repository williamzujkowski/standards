...
class HIPAAAuditLogAnalyzer:
                ...
                if user_role not in self.emergency_roles:
                    self.violations.append(
                        ComplianceViolation(
                            ...
                            description="After-hours PHI access by non-emergency user",
                            user_id=entry.user_id,
                            timestamp=entry.timestamp,
                            patient_id=entry.patient_id,
                            details={"hour": hour, "user_role": user_role, "resource": entry.resource_accessed},
                        )
                    )
            ...
            if entry.patient_id in self.vip_patient_ids:
                ...
                self.violations.append(
                    ComplianceViolation(
                        ...
                        description="Access to VIP patient record",
                        user_id=entry.user_id,
                        timestamp=entry.timestamp,
                        patient_id=entry.patient_id,
                        details={
            ...
            if entry.user_id in self.terminated_user_ids:
                self.violations.append(
                    ComplianceViolation(
                        ...
                        description="Access by terminated employee",
                        user_id=entry.user_id,
                        timestamp=entry.timestamp,
                        patient_id=entry.patient_id,
                        details={
                ...
                if ip1 != ip2 and (time2 - time1) < timedelta(hours=1):
                    self.violations.append(
                        ComplianceViolation(
                            ...
                            description="Access from different IPs within 1 hour",
                            user_id=user_id,
                            timestamp=time2.isoformat(),
                            details={"ip1": ip1, "ip2": ip2, "time_diff_minutes": (time2 - time1).total_seconds() / 60},
                        )
                    )
            ...
            if user_patient_id and entry.patient_id == user_patient_id:
                self.violations.append(
                    ComplianceViolation(
                        ...
                        description="Employee accessed own medical record",
                        user_id=entry.user_id,
                        timestamp=entry.timestamp,
                        patient_id=entry.patient_id,
                        details={"event_type": entry.event_type, "justification_required": True},
                    )
                )
    ...
    def analyze_audit_log_tampering(self):
        ...
        timestamps = sorted([datetime.fromisoformat(e.timestamp.replace("Z", "+00:00")) for e in self.audit_entries])
    ...
    def run_all_analyses(self):
        ...
        print("✓ After-hours access analysis complete")

        self.analyze_bulk_access()
        print("✓ Bulk access analysis complete")

        self.analyze_vip_access()
        print("✓ VIP access analysis complete")

        self.analyze_terminated_user_access()
        print("✓ Terminated user analysis complete")

        self.analyze_failed_logins()
        print("✓ Failed login analysis complete")

        self.analyze_geographical_anomalies()
        print("✓ Geographical anomaly analysis complete")

        self.analyze_self_access()
        print("✓ Self-access analysis complete")

        self.analyze_audit_log_tampering()
        print("✓ Audit log tampering analysis complete")
    ...
    def generate_summary_report(self):
        ...
        print(
            f"Audit Period: {self.audit_entries[0].timestamp if self.audit_entries else 'N/A'} to {self.audit_entries[-1].timestamp if self.audit_entries else 'N/A'}"
        )
