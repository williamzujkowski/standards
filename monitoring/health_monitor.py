...
class HealthMonitor:
    ...
    def run_comprehensive_health_check(self):
        ...
        self.logger.info(f"Health check completed. Overall status: {health_report['overall_status']}, Score: {health_report['health_score']}")
    ...
    def _check_standards_consistency(self):
        ...
        try:
            ...
            for std_id, std_info in standards.items():
                ...
                if os.path.exists(file_path):
                    try:
                        ...
                        if missing_sections:
                            inconsistent_structures.append(f"{std_info['full_name']}: missing {', '.join(missing_sections)}")
            ...
            check_result['issues'] = (
                [f"Missing standards files: {', '.join(missing_standards)}"] if missing_standards else [] +
                consistency_issues +
                inconsistent_structures
            )
    ...
    def _check_security(self):
        ...
        sensitive_patterns = [
            '*.key', '*.pem', '*password*', '*secret*', '.env*', 'id_rsa*'
        ]
