...
class PerformanceMonitor:
    ...
    def benchmark_search_operations(self):
        ...
        search_tests = [
            ...
            ('complex_grep', "grep -r -E '(security|authentication|authorization)' docs/ --include='*.md'"),
    ...
    def _calculate_health_score(self, report):
        ...
        git_success_rate = sum(1 for op in git_ops.values() if op.get('success', False)) / len(git_ops) if git_ops else 0
        ...
        script_success_rate = sum(1 for op in script_perf.values() if op.get('success', False)) / len(script_perf) if script_perf else 0
        ...
        search_success_rate = sum(1 for op in search_benchmarks.values() if op.get('success', False)) / len(search_benchmarks) if search_benchmarks else 0
    ...
    def continuous_monitoring(self, duration_minutes=60, interval_minutes=5):
        ...
        while datetime.now() < end_time:
            try:
                ...
                self.logger.info(f"Monitoring cycle completed. Health score: {report.get('health_score', {}).get('overall', 'N/A')}")
        ...
        summary = {
            ...
            'average_health_score': statistics.mean([r.get('health_score', {}).get('overall', 0) for r in reports]) if reports else 0,
            'reports': reports
        }
        
        summary_file = os.path.join(self.metrics_dir, f'continuous_monitoring_summary_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
        try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--interval', type=int, default=5, help='Interval between monitoring cycles (minutes)')
    ...
    if args.continuous:
        ...
        print(f"Continuous monitoring completed. Average health score: {summary.get('average_health_score', 'N/A'):.2f}")
    else:
