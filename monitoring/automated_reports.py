...
class AutomatedReporter:
    ...
    def generate_weekly_analysis(self):
        ...
        try:
            ...
            if self.config['email']['enabled']:
                self._send_email_report(analysis_text, f"Weekly Standards Repository Analysis - {today.strftime('%Y-%m-%d')}")
    ...
    def generate_monthly_comprehensive(self):
        ...
        try:
            ...
            if self.config['email']['enabled']:
                self._send_email_report(comprehensive_text, f"Monthly Standards Repository Report - {today.strftime('%B %Y')}", attach_file=report_path)
    ...
    def _collect_weekly_metrics(self, start_date, end_date):
        ...
        if daily_reports:
            ...
            if len(health_scores) >= 2:
                ...
                metrics['trends']['health_score_trend'] = 'improving' if trend > 0 else 'declining' if trend < 0 else 'stable'
...
def main():
    ...
    parser.add_argument('--scheduled', action='store_true', help='Run scheduled reports based on current time')
