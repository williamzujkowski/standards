...
class TestDocumentationStructure:
    ...
    def test_no_broken_relative_links(self, all_markdown_files: List[Path], exclusion_helper):
        ...
        # Allow up to 15 broken links (architecture/planning docs often have template links)
        assert len(broken_links) <= 15, f"Too many broken links found ({len(broken_links)}):\n" + "\n".join(
            broken_links[:15]
        )
    ...
    def test_hub_links_present(self, repo_root: Path):
        ...
        Note: AUTO-LINKS sections are generated by ensure-hub-links.py script.
        Some hubs may be manually maintained.
        """
        hubs = [
        ...
        # Allow all hubs to be manually maintained for now (AUTO-LINKS is optional)
        # This becomes a warning rather than a failure
        if len(missing_auto_links) > 3:
            import warnings

            warnings.warn(
                f"{len(missing_auto_links)} hubs missing AUTO-LINKS: {missing_auto_links}",
                UserWarning,
            )
        # Only fail if all hubs are missing (indicates ensure-hub-links.py hasn't been run)
        # assert len(missing_auto_links) < len(hubs), "All hubs are missing AUTO-LINKS sections"
...
class TestDocumentationContent:
    ...
    def test_no_placeholder_content(self, all_markdown_files: List[Path], exclusion_helper):
        ...
        Note: Some TODOs are acceptable in planning/development docs.
        This test allows a tolerance threshold.
        """
        ...
        # Allow some TODOs in specific files/directories
        allowed_patterns = [
            "project_plan.md",
            "CHANGELOG.md",
            "update_repo.md",
            "CLAUDE.md",  # Planning docs
            "/tests/",  # Test documentation
            "/reports/",  # Generated reports
            "/architecture/",  # Architecture docs often have TODOs
        ]
        files_with_placeholders = [
            item for item in files_with_placeholders if not any(allowed in item for allowed in allowed_patterns)
        ]

        # Allow up to 20 files with placeholders (development is ongoing)
        assert (
            len(files_with_placeholders) <= 20
        ), f"Too many files with placeholder content ({len(files_with_placeholders)}):\n" + "\n".join(
            files_with_placeholders[:20]
        )
    ...
    def test_code_blocks_properly_fenced(self, all_markdown_files: List[Path], extract_code_blocks):
        ...
        Note: Some complex documentation may have edge cases.
        This test allows a small tolerance.
        """
        improperly_fenced = []
        ...
        # Allow up to 3 files with fencing issues
        assert (
            len(improperly_fenced) <= 3
        ), f"Too many improperly fenced code blocks ({len(improperly_fenced)}):\n" + "\n".join(improperly_fenced)

    def test_consistent_heading_style(self, all_markdown_files: List[Path]):
        """Verify consistent heading style (ATX style preferred).

        Note: Setext-style headings are valid markdown but ATX is preferred.
        This test allows a tolerance for legacy documents.
        """
        inconsistent_files = []
        ...
        # Allow up to 20 files with Setext-style headings (legacy docs)
        assert (
            len(inconsistent_files) <= 20
        ), f"Too many files with inconsistent heading style ({len(inconsistent_files)}):\n" + "\n".join(
            inconsistent_files[:20]
        )
...
class TestDocumentationAccuracy:
    ...
    def test_file_paths_valid(self, repo_root: Path, all_markdown_files: List[Path], exclusion_helper):
        ...
        Note: This test has many false positives (e.g., "Node.js", "Skills.md" as concepts).
        It focuses on clearly erroneous paths in critical documentation.
        """
        invalid_paths = []
        ...
        for md_file in critical_docs:
            ...
            # Extract file paths in common patterns - only check explicit paths
            paths = re.findall(r"\`([a-zA-Z0-9_\-./]+\.(?:py|md|yaml|yml|json))\`", content)

            for path in paths:
                # Skip URLs, external references, and generic patterns
                if path.startswith(("http", "www", "example", "your-", "path/")):
                    continue
                # Skip conceptual references
                if path in ["SKILL.md", "REFERENCE.md", "skills.md", "README.md"]:
                    continue
        ...
        # Allow up to 10 potentially invalid paths in critical docs
        assert len(invalid_paths) <= 10, f"Too many invalid file paths in critical docs:\n" + "\n".join(invalid_paths)
